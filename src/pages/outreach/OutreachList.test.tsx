...
// ---------------------------------------------------------------------------
// T325 -- the row actions render in the row BODY, not Astryx's endContent slot.
// ---------------------------------------------------------------------------

describe('T325: the student row actions render in the row body, not the endContent slot', () => {
  // WHY THIS IS A STRUCTURAL TEST AND NOT A WIDTH TEST, stated so nobody
  // "improves" it into something jsdom cannot actually do: jsdom performs NO
  // LAYOUT, so it cannot see the 213px overflow this task fixes, and it
  // cannot see a regression either. The real evidence is a measurement in
  // real Chromium at 390x844 (baseline 213px -> 0px, recorded in
  // `verification-log.md`). This test pins the one thing jsdom CAN see: that
  // the actions are rendered inside the row's own body subtree -- the space
  // this component controls and where `wrap` engages -- rather than back in
  // Astryx's `flex-shrink: 0` slot, where wrapping can never fire.
  it('the "session details" action shares an ancestor with the row body text, not a separate slot', async () => {
    renderAsUser(STUDENT_OR_PARENT_USER, {
      loadData: defaultLoadOutreachData,
      resolveStudentId: async () => 'student-lena-osei',
    });
    ...
    const actionButton = Array.from(container.querySelectorAll('button')).find((button) =>
      /session details/.test(button.textContent ?? ''),
    );
    // Paired: prove the button exists at all before asserting where it is.
    // The first version of this fix measured overflow 0 *because the buttons
    // had vanished* -- a number alone is not evidence.
    expect(actionButton).toBeTruthy();
    expect(actionButton?.textContent).toContain('–'); // full accessible label kept (T131/T132).

    // The row body carries the location line. If the actions are in the body,
    // some ancestor of the button also contains that text. If they are back
    // in the endContent slot, no shared ancestor does.
    let ancestor: HTMLElement | null = actionButton?.parentElement ?? null;
    let sharesBodyAncestor = false;
    while (ancestor !== null) {
      if ((ancestor.textContent ?? '').includes('Riverside Park')) {
        sharesBodyAncestor = true;
        break;
      }
      ancestor = ancestor.parentElement;
    }
    expect(sharesBodyAncestor).toBe(true);
  });
});
