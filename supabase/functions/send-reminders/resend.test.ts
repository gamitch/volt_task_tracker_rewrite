// supabase/functions/send-reminders/resend.test.ts
//
// T051 -- unit tests for this function's own independently-reimplemented
// resend.ts (see that file's header for why it is not imported from
// send-invite/resend.ts). Constitution item 7's BLOCKER-class test-mode
// gate is the primary thing under test here -- same coverage shape as
// send-invite/resend.test.ts, written independently (this task cannot
// import send-invite/** at all, including its test files). Run with
// `deno test --allow-env`.
...
Deno.test('sendBrandedEmail: in test mode, never calls fetch and returns skipped_test_mode', async () => {
  ...
  try {
    const result = await sendBrandedEmail(
      { to: 'someone@example.com', subject: 'Reminder', html: '<p>hi</p>', from: 'VOLT Robotics <notifications@mail.voltfrc.org>' },
      'test',
    );
...
Deno.test('sendBrandedEmail: in production mode, calls fetch exactly once against api.resend.com with a Bearer header, never logs the key', async () => {
  ...
  globalThis.fetch = ((url: string | URL, init?: RequestInit) => {
    ...
    return Promise.resolve(new Response(JSON.stringify({ id: 'resend-msg-1' }), { status: 200 }));
  }) as typeof fetch;

  try {
    const result = await sendBrandedEmail(
      { to: 'someone@example.com', subject: 'Your session is coming up', html: '<p>hi</p>', from: 'VOLT Robotics <notifications@mail.voltfrc.org>' },
      'production',
    );
...
Deno.test('sendBrandedEmail: in production mode with a non-2xx Resend response, returns resend_error with the status, no throw', async () => {
  ...
  globalThis.fetch = (() => Promise.resolve(new Response(JSON.stringify({ message: 'invalid from address' }), { status: 422 }))) as typeof fetch;
