...
| **LIVE-006** | Mobile student RSVP row collapses its content | **T325 — VERIFIED 2026-08-04** | **CONFIRMED by direct measurement** (real Chromium, 390×844): **213px of horizontal overflow**, `scrollWidth` 603 vs `clientWidth` 390. **But the description is misleading** — nothing collapses, the row *overflows*, and it is **not** the RSVP `SegmentedControl`. It is the `endContent` action span (`flex-shrink: 0`, 563px inside a 342px `ListItem`). Root cause: the student view has no `isNarrow` branch at all, unlike the coach table. See the T325 ledger row. |
