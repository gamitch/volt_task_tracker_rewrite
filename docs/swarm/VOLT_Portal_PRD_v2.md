...
| **D-7** | Coach authority over student-authored records | **DECIDED BY GEORGE (2026-07-20): the coach is the ultimate authority.** Staff edits MAY overwrite or delete a student's own RSVP, and coach attendance corrections MAY remove QR-originated check-in rows outright. The T117/T118 "self-authored protection" rules (never delete/never overwrite a student's own RSVP; QR rows demoted to 'absent' instead of deleted) are REMOVED (T119). Simplified rules: expected-attendees checked → upsert `'going'` regardless of prior author/status; unchecked → delete `'going'` rows regardless of author (a student's explicit `'declined'`/`'maybe'` is left alone by an uncheck, since "not expected" isn't "they answered no"); attendance uncheck → plain DELETE regardless of `method`. Origin labeling (`responded_by`, `recorded_by`) remains for the activity feed — visibility, not veto. | George's words: "As coach I am ultimate authority and should be able to overwrite an RSVP or check-ins." Also §8 simplicity: one rule ("the checklist wins") instead of a provenance matrix. |
