...
- **MTG-11 Coach roll call & override:** each roster row in the console has `SegmentedControl` [Present | Late | Excused | Absent]. A coach tap upserts with `method='coach'`, `recorded_by=coach`, and ~~**always wins** over QR values (QR writes never overwrite a `method='coach'` row)~~. Rows update in real time via Supabase Realtime subscription on `attendance` for the session.
  - **⚠️ SUPERSEDED 2026-08-02 — the struck clause is no longer the rule. LAST WRITE WINS.** Owner ruling, recorded verbatim in `auto-mode-decisions.md` under *"George's ruling on MTG-11: LAST WRITE WINS, overturning coach precedence"* — cite that section, never paraphrase this note. **The case that broke the old rule:** a coach marks a student absent before they arrive; the student turns up late and scans the kiosk. Under coach-precedence the scan is discarded and the student stays `absent` while standing in the room. **The first clause is unchanged and was always correct** — a coach tap does write `method='coach'`. `method` means *"who set the value that is there now"*, not *"how did this student physically check in"*; there is deliberately no second field, so a row can never claim `method='qr'` while naming a coach in `recorded_by`. **Acceptance item 4 in §12 is superseded by the same ruling** and should be read as its inverse.
...
- **MTG-14 Student/parent meeting views:** `/meetings` for students = their own history (status per session) + participation %; parents see the same per linked student. Read-only.
  - **⚠️ Presentation superseded 2026-08-21 by MTG-01c** (the dated deviation above MTG-01). The history *table* is replaced by a hero card for the next meeting plus an attendance summary and history list; parents switch between linked students. **Read-only is unchanged**, and so is every datum listed here.
...
| `/meetings` | all | Card page (2026-08-21 deviation; was: List page) | `Card`, `TabList`, `Calendar`, `Badge`, `ProgressBar` | MTG-01 as amended by MTG-01a…h, MTG-14 |
...
4. ~~Coach override survives a subsequent QR write (MTG-11).~~ **SUPERSEDED 2026-08-02 — read as its inverse: a subsequent QR write DOES overwrite a coach value, because the later write is the current truth. Owner ruling, `auto-mode-decisions.md`, *"George's ruling on MTG-11: LAST WRITE WINS"*. See the annotation on MTG-11 itself.**
