...
configuration that three of four measured findings show is *not* the default.**
...
| 13 | Paused executor, advanced generation, stale mutations rejected | **PASS, both routes** | (i) stale *belief* — current token, wrong `p_expected_generation` → `stale_generation`; (ii) **fencing** — `advance_generation` bumps generation *and rotates `capability_hash`*, so the old token → `no_such_capability` and the **new** token still publishes. Row re-read field-by-field as unchanged in both. |
...
off.** `force row level security` binds the table *owner*; it does not defeat the
`BYPASSRLS` role *attribute*. Measured on 17.11: a `postgres`-owned
...
`nologin` test rig therefore makes *every* escalation assertion succeed. Measured
...
**What the four amount to.** Three of them are *defaults* — the GUC's
...
asserted values**. Enforcement is the *absence of grants*, which is stronger and
...
- **No RLS on the run tables.** Forced RLS with a `NOBYPASSRLS` owner was
  *measured* to deadlock `reserve_run` outright (`ERROR: 42501: new row violates
...
- **No extension dependency.** `sha256(bytea)` and `gen_random_uuid()` are PG
  built-ins. The hosted project *does* have `pgcrypto`, so this is now a
...
the majors is cosmetic and is *not* asserted anywhere: PG 17 adds the `MAINTAIN`
...
1. **The hosted project's role attributes.** Whether hosted `service_role`,
   `authenticated` and `anon` carry what the harness creates locally is
   **unmeasured** — and criterion 3's whole argument is a role-attribute
   argument. One query closes it, on the same owner channel that closed GAM-408.
   Carried by **GAM-414**. **No criterion-3 PASS may be quoted without this
   caveat in the same sentence.**
2. **Scenario 15's ambiguous case.** Only the *unreachable store* is exercised —
...
6. **Mutation 2's red is a hard `42P10`**, not a duplicate row: without the unique
   ...
   probe confirmed the *stronger* form separately — constraint dropped **and**
...
Plan §5.1: *"If that spike fails, stop."* **No criterion is FAIL, so the stop
...
| Follow-ups filed | GAM-414 (hosted `pg_roles`), GAM-415 (CI PG-version skew), plus the checker's MINOR-2/MINOR-3/NIT rows |
