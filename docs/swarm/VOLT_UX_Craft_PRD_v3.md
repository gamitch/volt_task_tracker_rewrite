...
| UXC-04 | Compact affordances; visible control text NEVER repeats the row title (title → `aria-label` only). **EXEMPTION (2026-07-28):** "View details – {title}" row links keep their full visible text. Astryx's own Link guidance forbids `label`/`aria-label` on text links ("prevents assistive technology from reading the actual link content") and forbids generic text like "read more"; T112 (Passed) established the current wording and `OutreachList.test.tsx:1592-1595`/`:1625` pin it. Applies to `OutreachList` and the mirrored `CalendarPage` link. Buttons are unaffected — the pattern there (visible short text + title in `label`) is correct and already shipped on Edit/Cancel. | MAJOR |
