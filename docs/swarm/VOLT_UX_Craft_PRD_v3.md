...
| UXC-02 | Coach event/meeting rows migrate `List`→**`Table`** with `pixel()`/`proportional()` tracks, grouped sections, expander. Zero x-drift. *Basis: PRD v1 §7.1 compact rows.* **SUPERSEDED FOR `/meetings` ONLY (2026-08-21, George)** — that page becomes a series-**card** page and has no rows to migrate; see the dated deviation above MTG-01 in `VOLT_Portal_PRD.md` (MTG-01a…h). **Unchanged and in full force on every other coach surface**, including `/outreach` and `/calendar`. Do not read this as a general retreat from `Table`. | MAJOR |
...
| UXC-07 | Collapsed coach rows **≤72px measured** (not "≥8 rows" — only 5 fixtures exist); ONE separation system (default: bordered row-cards); expander never out-weighs titles. **SUPERSEDED FOR `/meetings` ONLY (2026-08-21, George)** — a series card is deliberately taller than 72px and is not a row; see MTG-01a in `VOLT_Portal_PRD.md`. **Whether the ≤72px cap binds MTG-01b's schedule-panel session rows is explicitly NOT ruled on here** — no owner ruled on it, no reference figure exists to measure against, and UXC-07's severity column (`with UXC-02`) has no anchor on this page now that UXC-02 is superseded for it. Treat it as a **MINOR** target on those rows, and **where it conflicts with MTG-01g's ≥44px tap target, ≥44px wins**; GAM-448 raises a dispute if it turns out to bind. Everywhere UXC-07 already applied outside `/meetings`, it is unchanged. | with UXC-02 |
