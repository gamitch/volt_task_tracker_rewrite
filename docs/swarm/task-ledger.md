...
| T006 | E1 | AppShell + TopNav | worker-implementer | checker-accessibility | T002,T005 | 0 | Ready | Unblocked 2026-07-17 on T005's PASS full-ledger sweep (T002 and T005 both now Passed). Not yet dispatched. | No |
...
| T013 | E2 | Metric views (verbatim 8.4) | worker-implementer | checker-tests | T010,T012 | 0 | In Progress | Dispatched 2026-07-17 to worker-implementer per `docs/swarm/active/T013-worker-packet.md` (file-disjoint parallel dispatch alongside T015). Note for worker packet: T013's views are also expected to close T012's deliberately-deferred Trap 1 gap (`students` teammate name/team visibility for the leaderboard) — flagged explicitly in the packet as an open question, not to be solved unilaterally by reshaping the verbatim views. | No |
| T014 | E2 | NFR-03 metric-view fixture tests | worker-implementer | checker-tests | T013 | 0 | Blocked | None | No |
| T015 | E3 | Supabase Auth provider config | worker-implementer | checker-tests | T012 | 0 | In Progress | Dispatched 2026-07-17 to worker-implementer per `docs/swarm/active/T015-worker-packet.md` (file-disjoint parallel dispatch alongside T013). External blocker still applies (George's Google OAuth client credential does not yet exist) — scoped to authored/staged, secret-free config only, not end-to-end verification. | No |
