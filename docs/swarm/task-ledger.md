...
| T002b | E1 | Dark-mode `--color-on-accent` contrast fix (D005 corrective task) | worker-implementer | checker-accessibility | T002,T003 | 0 | Ready | Created 2026-07-19 by boss-arbiter ruling D005 (dispatch-ready, same-day-class — live WCAG AA shortfall on `/login`, the app's only reachable real page): dark-mode `Button variant="primary"` text measures 4.041:1 (`#0000B3` on `#9B7BFF`), below AA's 4.5:1 for 14px/500 text. Root cause is a PRD-internal DES-03-vs-DES-06 conflict (DES-03's raw dark-accent override silently invalidates Astryx's baked on-accent derivation — verified against installed `expandColorScale.ts`/`defineTheme.ts` source), not any prior task's worker/checker error. D005-authorized fix: exactly one added line in `volt.ts`'s tokens map — `'--color-on-accent': ['#FFFFFF', '#00008D']` (Astryx's own P[10] ramp stop, 4.818:1 verified) — plus lockstep regeneration of `theme.css`'s generated block. See detail block below and `dispute-log.md` D005 Rulings A–C/F. | No |
