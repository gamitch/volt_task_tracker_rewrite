...
| T004 | E1 | CI pipeline (typecheck/lint/unit/build + bundle budget) | worker-implementer | checker-tests | T001,T002,T003 | 0 | Ready | None | No |
| T005 | E1 | Router skeleton + route guards + deep-link redirect | worker-implementer | checker-reviewer | T001 | 1 | Needs Rework | FAIL/MAJOR (BLOCKER-class) on 1st check. Verified correct, no rework needed: all 13 PRD Section 7 routes present as stubs; RequireAuth/RequireRole guard logic independently verified correct via checker's own 6-test suite (unauth→/login; wrong-role→/ + exact toast "You don't have access to that page."); NAV-08 intended-URL round-trip verified for both login() and loginWithGoogle() paths; main.tsx/App.tsx confirmed correctly unwired (deferred to T006 as intended); build/typecheck/lint all clean (only expected non-blocking react-refresh/only-export-components warnings); react-router-dom@7.18.1 the only new dependency, peer range satisfied by React 19, no --legacy-peer-deps. Sole failure (K1): `/kiosk/:sessionId` stubbed as a fully public/unguarded route, with a doc comment incorrectly claiming the PRD is silent on kiosk auth — PRD Section 7's route table explicitly assigns `/kiosk/:sessionId` to coach/admin, and SEC-04 states "no public pages"; constitution flags SEC-04/kiosk surfaces as BLOCKER-class. Rework: guard the route with RequireAuth + RequireRole(['coach','admin']), add 'coach' to guards.tsx's Role union (K2, needed for K1's fix to compile), and correct the misleading doc comment. See `docs/swarm/active/T005-latest-failure.md` for full detail. K3 (toast-during-render NIT) logged only, no action this attempt. | No |
...
| T011 | E2 | Migration: support tables + audit triggers (DATA-02) | worker-implementer | checker-tests | T010 | 0 | In Progress | Worker packet dispatched 2026-07-17 (parallel dispatch alongside T005 checker-reviewer and T003 checker-tests dispatches). | No |
