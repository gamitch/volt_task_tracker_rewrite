...
| T007 | E1 | SideNav (role-filtered) + outreach badge scaffold | worker-implementer | checker-accessibility | T006 | 1 | In Progress | FAIL (attempt 1) — BLOCKER, with a concrete in-scope fix already identified: NAV-03/04/07/BEH-04 compliance, Astryx prop sourcing (including independently re-run CLI cross-checks for `SideNavItem`/`SideNavSection`'s undocumented props), keyboard/focus, and the `AppShell.tsx` D001 wiring boundary all independently verified correct — task fails solely on a real hard-navigation/session-loss defect: `SideNavItem` renders a plain `<a href>` with no router-aware link component, so every click (confirmed via both mouse and keyboard/Enter) triggers a full browser reload that resets the in-memory `AuthProvider` session and bounces back to `/login`, reproducing across all 5-7 items in every role tested. Checker judged this materially worse than T006's already-Passed TopNav precedent (SideNav is 100% broken on its only interaction surface, vs. TopNav's single edge-case wordmark link) and, critically, found the fix is fully in-scope: `SideNavItem`'s own CLI-confirmed `as: LinkComponentType` prop (present in the same CLI output the worker already ran, one row past what it cited) wired to React Router's already-allowlisted `Link` — checker empirically proved this eliminates the reload entirely (temporary patch, verified, then reverted) before rendering the verdict. Recommended rework by the same worker (attempt 2 of 3), not a dispute, since no forbidden file needs touching. Rework packet dispatched. | No |
