...
| T016a | E3 | Wire real LoginPage into router.tsx | worker-implementer | checker-tests | T016 | 0 | In Progress | Dispatched 2026-07-18 to worker-implementer per `docs/swarm/active/T016a-worker-packet.md`. User-approved standalone corrective task closing the router.tsx wiring gap both T016's worker and checker flagged (see `docs/swarm/verification-log.md` T016 entry and the state-summary.md "router.tsx wiring gap" Current Risks entry). Scope: remove router.tsx's inline placeholder `LoginPage()` + its `signInAs`/`continueWithGoogle` helpers, import the real component from `../pages/login`, verify live via Playwright (real render, full sign-in round trip, NAV-08 intended-URL preservation). `src/pages/login/**` and `guards.tsx` forbidden — mechanical wiring only, no real Supabase auth in scope. | No |
