...
| T006 | E1 | AppShell + TopNav | worker-implementer | checker-accessibility | T002,T005 | 0 | In Progress | Dispatched 2026-07-18 to worker-implementer per `docs/swarm/active/T006-worker-packet.md`. Packet scopes AppShell.tsx + TopNav.tsx (new) plus a wiring edit to App.tsx (BrowserRouter+AuthProvider+LayerProvider+Theme+AppShell+AppRoutes) since T005 left main.tsx/App.tsx deliberately unwired for this task to close; router.tsx and guards.tsx remain read-only/forbidden. Awaiting worker output. | No |
