...
| T006 | E1 | AppShell + TopNav | worker-implementer | checker-accessibility | T002,T005 | 1 | In Progress | FAIL (attempt 1) — BLOCKER, but NOT on T006's own deliverable: checker-accessibility independently verified AppShell.tsx/TopNav.tsx/App.tsx clean on every axis (NAV-01/02 compliance, Astryx prop cross-check, DES-17 keyboard/focus via a real scratch-test keyboard-event harness, role-gating, null-user safety, forbidden-file boundary, build/typecheck/lint/format:check). Failure is structural: wiring App.tsx into a real router+AppShell tree (mandated by T005's own module doc, not worker scope creep) broke the pre-existing, already-Passed `theme.smoke.test.tsx` (T002a, outside T006's Allowed Files) two independent ways — (a) `matchMedia` not polyfilled anywhere in the project, (b) even with a polyfill, the test's `'VOLT Team Portal'` assertion is now permanently stale since unauthenticated sessions now correctly redirect to `/login`. Confirmed live on real GitHub Actions CI (required `npm run test` gate red on every push since commit e20b8d1). Checker judged this unfixable within T006's own Allowed Files (fix requires editing the forbidden test file) and recommended NOT burning a T006 rework attempt — escalate straight to boss-arbiter instead, same pattern as D001/D002. Dispute filed; boss-arbiter ruled D003 (2026-07-18): worker not at fault, no T006 rework; blocker reassigned to corrective task T006a. T006 stays In Progress pending T006a's PASS (which serves as T006's re-verification — no separate checker-accessibility re-run needed per D003 Ruling F). | No |
| T006a | E1 | Fix CI test regression from T006 wiring (D003 corrective task) | worker-implementer | checker-tests | T006 | 0 | In Progress | Dispatched 2026-07-18: worker packet issued at `docs/swarm/active/T006a-worker-packet.md` per boss-arbiter ruling D003 (`docs/swarm/dispute-log.md`), transcribing D003 Rulings C/D verbatim as scope. Attempt 1 underway; awaiting worker-implementer output and checker-tests review. Real GitHub Actions CI is red on the required `npm run test` gate pending this fix. | No |
...
| T014 | E2 | NFR-03 metric-view fixture tests | worker-implementer | checker-tests | T013 | 0 | Ready | Unblocked 2026-07-17 by T013 PASS (sole dependency now Passed). | No |
...
| T017 | E3 | `send-invite` Edge Function | worker-implementer | checker-reviewer | T009,T012 | 0 | Ready | Unblocked 2026-07-17 on T012's PASS full-ledger sweep (T009 and T012 both now Passed). Not yet dispatched. | No |
...
| T032 | E5 | `checkin` Edge Function (HMAC rotating token) | worker-implementer | checker-tests | T010,T012 | 0 | Ready | Unblocked 2026-07-17 on T012's PASS full-ledger sweep (T010 and T012 both now Passed). Not yet dispatched. | No |
...
| T061 | E10 | Schema verification + mapping doc copy (MIG-01/02) | worker-implementer | checker-reviewer | T009,T010,T011 | 0 | Ready | Unblocked 2026-07-17 on T011's PASS full-ledger sweep (T009, T010, T011 all now Passed). Not yet dispatched. | No |
