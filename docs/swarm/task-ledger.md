...
| T007 | E1 | SideNav (role-filtered) + outreach badge scaffold | worker-implementer | checker-accessibility | T006 | 0 | Ready | Unblocked 2026-07-18 on T006/T006a's PASS full-ledger sweep (sole dependency T006 now Passed). Not yet dispatched. | No |
...
| T016 | E3 | `/login` screen | worker-implementer | checker-accessibility | T006,T015 | 0 | In Progress | Dispatched 2026-07-18 to worker-implementer per `docs/swarm/active/T016-worker-packet.md`. Foreman confirmed via direct read of `router.tsx` that the current `/login` placeholder is an inline `LoginPage()` function defined in `router.tsx` itself, NOT under `src/pages/login/**` (that directory does not exist yet) — worker packet flags this as an open scope/dispute question (router wiring gap) rather than letting the worker guess, and separately flags that no shared Supabase client/real-auth wiring into `guards.tsx` exists yet either. Worker is building the full component under `src/pages/login/**` regardless, with evidence via a standalone render harness, and is instructed to raise both gaps as dispute candidates in its output rather than touch forbidden files. | No |
