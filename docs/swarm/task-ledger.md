...
| T008 | E1 | MobileNav drawer + Student Home live-card slot | worker-implementer | checker-accessibility | T006,T007 | 1 | In Progress | Attempt 1 (2026-07-18) built `MobileNav.tsx`/`StudentHomeSlot.tsx` plus the packet-mandated `AppShell.tsx`/`TopNav.tsx` wiring correctly per spec — but the worker discovered and correctly did not work around a real defect: the packet-mandated `mobileNav={<MobileNav />}` ReactNode shorthand actually disables `MobileNavToggle` entirely in the installed `@astryxdesign/core@0.1.6` (confirmed via source read, CLI template output, and live testing), contradicting astryx-api.md's own prose. Escalated rather than improvised around, per constitution. **Boss-arbiter ruling D004 (2026-07-18, `docs/swarm/dispute-log.md`):** independently verified the worker's diagnosis against the installed library source and found one more thing the worker missed — authorized fix is `mobileNav={{ content: <MobileNav /> }}` (the `MobileNavConfig` object form) in `AppShell.tsx`, but this forces `TopNav` into a "mobile-bar" render mode that auto-injects its own `MobileNavToggle`, making T008's `TopNav.tsx` edit dead code; that edit must be **reverted** back to T006's Passed state, not amended. `astryx-api.md` corrected (not log-only) with two marked D004 annotations documenting the real `MobileNavConfig` shape, since checker item-2 compliance depends on it. Scoped continuation of T008 (same worker/checker, attempt counter stays at 1 — worker not at fault, no FAIL recorded, matching the D001 precedent for a correct escalation). Rework packet pending dispatch — paused here for session-budget reasons; D004's Rulings A-C are concrete enough to build the packet directly from without further judgment calls. | No |
