...
| T007 | E1 | SideNav (role-filtered) + outreach badge scaffold | worker-implementer | checker-accessibility | T006 | 1 | In Progress | Dispatched 2026-07-18 via `docs/swarm/active/T007-worker-packet.md`. **Scope note:** Allowed Files for this dispatch include `src/app/AppShell.tsx` in addition to the ledger's original `src/components/nav/SideNav.tsx`, scoped strictly to the single wiring edit (`sideNav={undefined}` → `sideNav={<SideNav />}`, plus its one import line and an optional stale-comment fix) — nothing else in that file. Reason: without this, `SideNav` would build and typecheck but never actually render in the app, reproducing the exact unlisted-but-load-bearing wiring gap that caused dispute D003 on T006. Adding it up front here avoids repeating that dispute. See the worker packet for the full acceptance criteria, the K2 `Role`-union gap workaround (student/parent visibility collapses to a two-bucket admin/coach-vs-not check, no `guards.tsx` edit needed), and the SideNav sub-component prop-table gap (astryx-api.md has no Props table for `SideNavItem`/`SideNavHeading`/`SideNavSection`/`SideNavCollapseButton` — CLI cross-check required and disclosed). | No |
