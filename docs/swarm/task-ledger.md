...
| T008 | E1 | MobileNav drawer + Student Home live-card slot | worker-implementer | checker-accessibility | T006,T007 | 1 | In Progress | Dispatched 2026-07-18, attempt 1. Allowed Files expanded beyond the ledger's original two-file scope (`MobileNav.tsx`, `StudentHomeSlot.tsx`) to include two additional scoped wiring edits, resolved in `docs/swarm/active/T008-worker-packet.md` before dispatch (not left for the worker to discover mid-task, per the D003/T007 lesson): (1) `src/app/AppShell.tsx`, scoped to changing `mobileNav={false}` to `mobileNav={<MobileNav />}` plus one import — same pattern as T007's `sideNav` wiring edit, otherwise the drawer never renders in the real app. (2) `src/components/nav/TopNav.tsx` (T006, already Passed), scoped to adding one import (`MobileNavToggle`) and one prop (`startContent={<MobileNavToggle />}`) — astryx-api.md documents `MobileNavToggle` as the only sanctioned trigger mechanism ("Inside AppShell, use MobileNavToggle as the trigger; it reads state from context automatically", line 4698) and `TopNav` has no dedicated mobile-menu-button prop, so satisfying NAV-05's literal "triggered from TopNav" requirement has no narrower path than a one-line addition inside `TopNav.tsx` itself. This is a new category of precedent beyond T007 (T007 avoided touching `TopNav.tsx` entirely via SideNav's bare `collapsible` prop; no equivalent escape hatch exists here). `StudentHomeSlot.tsx` scope also narrowed explicitly in the packet to an isolated, unwired placeholder component only (no Student Home page, no `router.tsx` edit) since T054 (real Student Home) remains unstarted with unmet dependencies. | No |
