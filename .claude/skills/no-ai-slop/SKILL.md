---
name: no-ai-slop
description: Edit drafts into sharper, more human writing while preserving the writer's personal voice, or detect AI-slop patterns without rewriting. Use when the user wants a draft clearer, more direct, more opinionated, or less AI-sounding, or asks whether writing reads as AI. In this repo, also use before committing prose an agent wrote for a human to read — ledger findings, dispute rulings, verification-log narratives, PR bodies, PRD sections, and user-facing copy under PRD DES-14…16.
---
...
Most prose here is written by an agent and read by a human: findings in
`docs/swarm/task-ledger.md`, narratives in `verification-log.md`, rulings in
`dispute-log.md`, PR bodies, and product copy. That is exactly the writing that
drifts into the patterns above — importance puffery on a one-line fix, colon
reveals in a finding, a fake-profound kicker at the end of a completion report.

Run the **detect** job on a PR body or a ledger finding before it lands. Run the
**edit** job only where the rules below allow it.

Three repo rules override the defaults:

- **Never edit a recorded claim into a stronger one.** Ledger rows, verification
  entries, and dispute rulings are evidence. Tightening "the loader returns an
  empty array for a student with no memberships" into something punchier is only
  legitimate if the shorter sentence says the same thing. If an edit changes what
  was claimed, measured, or ruled, it is a falsified record, not a better
  sentence. Cut filler; leave the claim, the SHA, the counts, and the file:line
  citations untouched.
- **Product copy answers to the PRD.** DES-14…16 prescribe sentence case, named
  actions, and exact empty/error strings. Where the PRD prescribes a string, the
  PRD wins — constitution item 1 puts PRD requirement IDs above every other
  source. Do not "improve" a prescribed string; file a task if it reads badly.
- **Do not run the edit job on `constitution.md`, the PRD files, or migration
  files.** Those are governed documents. Detect is fine; rewriting is a change
  that needs its own task and approval.

One caution specific to this codebase: this skill bans "robust" and tells you to
cut hedging adverbs. Both rules are about *prose*. Neither applies inside code,
test names, or a quoted error message.

## Provenance

Vendored from [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop)
(MIT, © 2026 Peter Yang). `SKILL.md` above the "In this repo" heading and
`eval.md` are upstream text, unmodified except for the added repo guidance in
the `description` and this section. Re-sync by diffing against
`skills/no-ai-slop/` upstream.
